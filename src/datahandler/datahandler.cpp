...
Datahandler::Datahandler::Datahandler(const std::string& _file_format){
    file_format = _file_format;
}

void Datahandler::Datahandler::set_file_format(const std::string& _file_format){
    file_format = _file_format;
}

void Datahandler::Datahandler::set_directory(const std::string& _file_directory){
...
void Datahandler::Datahandler::import_model_info_to(
    Model::Model* model_ptr
){
    std::ifstream fin;
    std::string file_name = file_directory + "/" + file_format + "_info.txt";
    fin.open(file_name);
...
void Datahandler::Datahandler::import_layer_to(Model::Model* model_ptr){
    uint index = 0;
    uint layer_num = model_ptr -> layer_num;
    std::string weight_file, bias_file;
    std::ifstream fin_weight, fin_bias;

    for(; index < layer_num; ++index){
        weight_file = file_directory + "/" + file_format + std::to_string(index + 1) + ".txt";
        bias_file = file_directory + "/" + file_format +std::to_string(index + 1) + "_bias.txt";

        fin_weight.open(weight_file);
        fin_bias.open(bias_file);
